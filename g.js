...
// self-recursive no-op mock
var PropTypesMock = function PropTypesMock() {
  return PropTypesMock;
};('array,bool,func,number,object,string,symbol,any,arrayOf,' + 'element,instanceOf,node,objectOf,oneOf,oneOfType,shape,exact').split(',').forEach(function (type) {
  PropTypesMock[type] = PropTypesMock;
});
...
var preactSpecificProps = ['autocomplete', 'autofocus', 'class', 'for', 'onDblClick', 'onSearch', 'slot', 'srcset'];

var PropTypes = void 0;

{
  // add only if there's no polyfill provided
  if (!React__default.Children) {
    React__default.Children = Children;
  }
  if (!React__default.PropTypes) {
...
{
  reactProps.push.apply(reactProps, toConsumableArray(preactSpecificProps));
}
